...
export default class ProfileCardInfo extends Component {
  ...
  render() {
    ...
    const mobileElement = (!infoList.contact)?null:
      <li className="user-card__info-elem">
        <svg className="user-card__info-icon" viewBox="0 0 24 24">
    ...
    return (
    	<StickyContainer className={classnames('user-card', this.props.className)}>
          ...
          isActive={this.props.isActive}>
	    		<ProfileAvatar className="user-card__user-avatar" src={infoList.profilepicture} />
		        <h2>
			        <div className="user-card__name">{infoList.name}</div>
			        <div className="user-card__role">{infoList.role}</div>
		        </h2>
